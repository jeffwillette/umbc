...
# NOTES:
# section 4.3 of the paper says they only set relaxed=True for the
# autoencoder model
...
class FSPool(nn.Module):
    ...
    def forward(self, x, n=None):
        ...
        # NOTE: this model is expecting to have set size as the last dimension,
        # so we need to move the set dimension to the end which is different
        # than most set encoders.
        x = x.transpose(1, 2)
        ...
        if hasattr(self, "hook"):
            x1 = x + (1 - mask).float() * -99999
            x2, p = x1.sort(dim=2, descending=True)
            x3 = (x2 * weight * mask.float()).sum(dim=2)

            self.hook(self, [x], x1)
            self.hook(self, [x1], x2)
            self.hook(self, [x1], p)
            self.hook(self, [x2], x3)
    ...
    def determine_weight(self, sizes):
        ...
        if hasattr(self, "hook"):
            index = (self.n_pieces * sizes).unsqueeze(1)
            index_exp = index.expand(index.size(
                0), weight.size(1), index.size(2))
            ...
            self.hook(
                ...
                weight.expand(sizes.size(0), weight.size(1), weight.size(2))
            )
            self.hook(self, [index], index_exp)
            self.hook(self, [frac], (1 - frac) * left + frac * right)
...
if __name__ == '__main__':
    pool = FSPool(2, 1)
    x = torch.arange(0, 2*3*4).view(3, 4, 2).float()
    print('x', x, x.size())
